...
def LoadGui(name):
    ...
    for message in save_dic[name].keys():
        ...
        image_address = save_dic[name][message][12]
...
def SaveGui(gui_name):
    ...
    for message in message_table_list:
        ...
        save_dic[gui_name][message][12] = image_address
...
'''
    if data_dic[message][7] != 'Cyclic':
        if data_dic[message][11] == 'c':
            bus1.send(can.message(arbitration_id=self.message_dic[self.cur_cell.text()][5],
                                  data=self.message_dic[self.cur_cell.text()][9],
                                  extended_id=False))
        else data_dic[message][11] == 'm':
            bus2.send(can.message(arbitration_id=self.message_dic[self.cur_cell.text()][5],
                                  data=self.message_dic[self.cur_cell.text()][9],
                                  extended_id=False))
'''
