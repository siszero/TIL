...
class WorkerC(QtCore.QThread, QtCore.QObject):  # Worker Thread
    def __init__(self, message_dic, bus, isFD = True):
        super().__init__()
    ...
    async def async_main(self):
        while True:
            for i in [asyncio.create_task(self.signal_Can(key)) for key in self.message_dic.keys()
                      if (self.message_dic[key][7] == 'Cyclic')]:
                await i
        ...
        while True:
            ...
            if boFind is True:
                if crcKey is not None:
                        ...
                        if 'AlvCnt' in newKey:
                            ...
                            if self.message_dic[key][8] == 8:
                                ...
                                if alvCntIdx == 256:
                                    alvCntIdx = 0
                            newMsgDic[newKey] = alvCntIdx
                            self.message_dic[key][13] = alvCntIdx
                            break
                    ...
                    encode_msg = bytes(self.select_gui_dic[key][9])
                if self.select_gui_dic[key][1] == 'ON':
                    try:

                        # outStr = ""
                        # for subkey, subitem in newMsgDic.items():
                        #     outStr += subkey+':'+str(subitem) +'/'
                        # print("Send msg: {0} / {1}".format(self.message_dic[key][5], outStr))
                        self.bus.send(
                            can.Message(arbitration_id=self.message_dic[key][5], data=encode_msg, is_extended_id=False,
                                        is_fd=self.isFD))
                    except:
                        print("fail: {0}".format(self.message_dic[key][5]))
                    # print(time.time() - self.start, key, self.message_dic[key][6] / 1000, self.select_gui_dic[key][9])
            else:
                None
            if self.message_dic[key][6] is not None:
                await asyncio.sleep(self.message_dic[key][6] / 1000)


class WorkerM(QtCore.QThread, QtCore.QObject):  # Worker Thread
